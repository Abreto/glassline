...
- `GLASSLINE_CONTROL_TOKEN` is a second authorization boundary for writes, not identity authentication and not a replacement for the external access layer. The launchd installer stores it in the user plist with mode `0600` when configured.
- The launchd installer validates and propagates an optional `GLASSLINE_ALLOWED_HOSTS` value while keeping `HOST=127.0.0.1`. It may be combined with `GLASSLINE_CONTROL_TOKEN` in the installation environment; either value can be omitted independently.
